...
export default function Register() {
  ...
  useEffect(() => {
    api.get('/facilities')
      ...
      .catch(() => {}); // silent fail — dropdown will be empty
  }, []);

  const lgas = state ? NigerianLGAs[state] || [] : [];
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const selectedFacility = facilities.find(f => String(f.id) === facilityName || f.name === facilityName);
      await api.post('/auth/register', {
        name: username,
        facility_name: selectedFacility?.name || facilityName,
        password,
        state,
        lga
      });
    ...
    <div className="min-h-screen bg-white dark:bg-gray-900 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-extrabold text-green-600 dark:text-green-400 tracking-tight">ECEWS CONSUMABLE TRACKER</h1>
          <p className="text-sm text-green-700 dark:text-green-300 mt-1">Create your account</p>
        </div>
        ...
        <div className="card p-8">
          <form onSubmit={handleSubmit} className="space-y-5">
           ...
           <p className="text-center text-xs text-white/80 mt-4">
             Already have an account? <Link to="/login" className="text-white font-semibold hover:underline underline-offset-2">Sign in</Link>
           </p>
